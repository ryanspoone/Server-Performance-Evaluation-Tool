...
def table(system_info, avx512=None):
    ...
    if system_info.l1dCache:
        display += __row_helper(
            "L1 Data Cache:",
            prettify.byte_size(system_info.l1dCache, suffix="B"))
    ...
    if system_info.osName and system_info.osVer:
        display += __row_helper("Operating System:",
                                system_info.osName,
                                unit=system_info.osVer)
    if system_info.archBits:
        display += __row_helper("Architecture:",
                                system_info.archBits,
                                unit="bit",
                                seperator="-")
    ...
    if system_info.memoryFrequency:
        display += __row_helper("Memory Frequency:",
                                system_info.memoryFrequency,
                                unit="MHz")

    display += seperator

    if system_info.gccVer:
        display += __row_helper("GNU Compiler Collection (GCC):",
                                system_info.gccVer)
